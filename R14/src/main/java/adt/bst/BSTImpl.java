...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private BSTNode<T> search(BSTNode<T> node, T element) {
		...
		if (element.compareTo(node.getData()) < 0) {
			if (!node.getLeft().isEmpty())
				result = this.search((BSTNode<T>) node.getLeft(), element);
			else
				result = node;
		} else {
			if (!node.getRight().isEmpty())
				result = this.search((BSTNode<T>) node.getRight(), element);
			else
				result = node;
		}
		if (result.isEmpty())
	...
	public void insert(T element) {
		if (element == null)
			return;
		this.insert(root, element, (BSTNode<T>) root.getParent());
	}

	protected BSTNode<T> insert(BSTNode<T> node, T element, BSTNode<T> parent) {
		BSTNode<T> nodeAt = node;
		if (node.isEmpty()) {
			...
			if (element.compareTo(node.getData()) < 0) {
				nodeAt = this.insert((BSTNode<T>) node.getLeft(), element, node);
			} else if (element.compareTo(node.getData()) > 0) {
				nodeAt = this.insert((BSTNode<T>) node.getRight(), element, node);
			}
		}
